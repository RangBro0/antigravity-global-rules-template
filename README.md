...
본 템플릿 패키지는 사용자의 사용 목적 및 환경에 맞춰 **선택식 수동 설치(Custom)**와 **일괄 자동 설치(Auto)**의 두 가지 설치 모드를 지원합니다. 

### 📌 1. 설치 모드 선택 (Custom vs Auto)

* **Custom 모드 (선택식 수동 설치 - 권장)**:
  - 깃허브 계정이 없거나 3자 동기화를 원치 않고, **필요한 스킬 및 규칙만 선택하여 로컬에 실장하고 싶을 때** 가장 적합합니다.
  - 스크립트 실행 중 각 스킬/플러그인 항목마다 **용도, 목적, 필수 요건**을 화면에 친절히 보여준 뒤 설치 여부(y/n)를 물어봅니다.
  - 사용자가 선택하지 않은(스킵한) MCP 서버는 설정 파일(`mcp_config.json`)에서 자동으로 제거되어 불필요한 빌드/실행 에러를 방지합니다.
* **Auto 모드 (일괄 자동 설치)**:
  - 개별적인 선택 과정 없이 템플릿의 모든 전역 규칙, 스킬, 개발 플러그인을 기본값으로 한 번에 적용하여 즉시 동기화 설치합니다.
...
1. **저장소 클론**:
   본 저장소 코드를 내려받습니다 (또는 zip 파일로 수동 다운로드 후 압축 해제).
   ```powershell
...
2. **동기화 스크립트 실행**:
   파워쉘(PowerShell) 콘솔에서 아래 설치 명령을 실행하고, 화면의 지시에 따라 모드를 선택합니다:
   ```powershell
   .\sync_config.ps1 -Mode Pull
   ```
   - **경로 감지 및 토큰 주입**: 스크립트 실행 시 구글 드라이브 마운트 문자(`G:`, `H:`, `I:`)와 `GITHUB_PERSONAL_ACCESS_TOKEN` 환경 변수를 자동 탐색하여 주입합니다. 만약 감지되지 않을 경우 입력을 요청하며, 구글 드라이브 연동을 원치 않을 시 엔터를 입력해 건너뛸 수 있습니다. (건너뛸 경우 `%USERPROFILE%\.gemini\config\data` 로컬 전용 보관함이 구성됩니다.)
