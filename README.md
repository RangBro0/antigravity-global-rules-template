...
2. **동기화 스크립트 실행**:
   ...
   ```
    - **설치 모드 지원 (Custom vs Auto)**: 스크립트 실행 직후 사용자가 직접 설치 방식을 제어할 수 있습니다.
      * **Custom 모드 (선택식 수동 설치 - 권장)**: 깃허브 계정이 없거나 원격 동기화를 원치 않고, 필요한 일부 스킬/규칙만 선별하여 로컬에 설치하고 싶을 때 매우 유용합니다. 스크립트가 각 스킬/플러그인 항목마다 **용도, 목적, 필수 사양**을 친절히 안내한 후 설치 여부(y/n)를 직접 물어봅니다. 선택하지 않은 MCP 도구는 설정 파일(`mcp_config.json`)에서 자동으로 제외되어 불필요한 빌드/실행 에러를 유발하지 않습니다.
      * **Auto 모드 (일괄 자동 설치)**: 개별 선택 없이 템플릿의 모든 규칙, 스킬, 플러그인을 기본값으로 한 번에 적용하여 설치합니다.
    - **경로 감지 및 토큰 주입**: 스크립트가 로컬 PC의 구글 드라이브 마운트 문자(`G:`, `H:`, `I:`)와 `GITHUB_PERSONAL_ACCESS_TOKEN` 환경 변수를 먼저 탐색하고, 감지되지 않으면 직접 입력을 유도합니다. 구글 드라이브 연동을 원치 않을 경우 엔터를 쳐서 건너뛸 수 있으며, 이 경우 로컬 데이터 보관 폴더(`%USERPROFILE%\.gemini\config\data`)를 자동 생성하여 로컬 단독 실행이 가능해집니다.
